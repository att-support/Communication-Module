...
std::string BinData::GetString(const unsigned char*& srcData, int length)
{
	const char* p = (const char*)srcData;
	std::string str(p, length);
	srcData += length;
	return str;
}
...
void BinData::SetString(const std::string& data, unsigned char*& dstData)
{
	strncpy((char*)dstData, data.c_str(), data.size());
	dstData += data.size();
}
