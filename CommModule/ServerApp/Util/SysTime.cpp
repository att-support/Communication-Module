...
uint64_t SysTime::GetNtpTime()
{
	...
	_timeb tv;
	_ftime(&tv);
	//tmp = gmtime(&tv.time);	//UTC
	tmp = localtime(&tv.time);
	i32nsec = tv.millitm * 1000000;
#else
	struct timeval tv;
	struct tm tmpTm;
	gettimeofday(&tv, NULL);
	localtime_r(&tv.tv_sec, &tmpTm);
	tmp = &tmpTm;
	i32nsec = m_tv.tv_usec * 1000;
#endif
	time_t dt = mktime(tmp);
	uint64_t totalSec = dt + 2208988800UL;
	uint32_t i32Fraction = ((uint64_t)i32nsec << 32) / 1000000000;
	uint64_t timestamp = ((totalSec << 32) & 0xFFFFFFFF00000000) | (i32Fraction & 0x00000000FFFFFFFF);
	return timestamp;
}
